...
The weighted average of these two values gives us our final DC Verdet constant

```{python}
C_dc_err = np.sqrt(1/(1/dc_slope1[1]**2 + 1/dc_slope2[1]**2))
C_dc = (dc_slope1[0]/dc_slope1[1]**2 + dc_slope2[0]/dc_slope2[1]**2) * C_dc_err**2
print(f"DC Verdet Constant: {C_dc:.4f} ± {C_dc_err:.4f} rad/T·m")
```
...
print(f"Theta 0: {theta_0} degrees")
...
print(f"  Current: {I:.4f} ± {I_std:.4f} A")
```
...
C_ac = dTheta/(k*I*L)
C_ac_err = C_ac * np.sqrt( (V_std/V)**2 + (I_std/I)**2 )
print(f"AC Verdet Constant: {C_ac:.4f} ± {C_ac_err:.4f}")
```
...
noise_mean = original_noise.mean()/(2*np.sqrt(2)) 
noise_std = original_noise.std()/(2*np.sqrt(2))
print(f"Original Noise (RMS): {noise_mean:.4f} ± {noise_std:.4f} mV")
```

It seems like our standard deviation is just about half our noise, meaning we actually are more accurate than our noise level, which is good.
